...
public class EnvConroller : MonoBehaviour
{
    ...
    private float ball_x_val_old = 0f;
    ...
    private void FixedUpdate()
    {
        ...
        else
        {
            ...
            if (ball_x_val_old < 0 && RbBall.velocity.x > 0)
            {
            ...
            ball_x_val_old = RbBall.velocity.x;

            if (Ball.transform.localPosition.x < -11f)
            {
            ...
            else if (Ball.transform.localPosition.x > 11f)
            {
                Agent_A.AddReward(1f);
                Agent_B.AddReward(-1f);
                Agent_A.EndEpisode();
    ...
    private void ResetScene()
    {
        ...
        float rand_num = UnityEngine.Random.Range(-1f, 1f);
